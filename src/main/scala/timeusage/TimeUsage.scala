...
object TimeUsage {
  ...
  def read(resource: String): (List[String], DataFrame) = {
    ...
    // Get the file contents using a different method because the prescribed method does not work
    // on my Windows PC
  ...
  def row(line: List[String]): Row =
    Row(line(0) :: line.tail.map(_.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primaryCols = columnNames.filter(c => c.startsWith("t01") || c.startsWith("t03") || c.startsWith("t11") ||
      c.startsWith("t1801") || c.startsWith("t1803"))
      .map(new Column(_).as[Double])

    val workingCols = columnNames.filter(c => c.startsWith("t05") || c.startsWith("t1805"))
      .map(new Column(_).as[Double])

    val otherCols = columnNames.filter(c => c.startsWith("t02") || c.startsWith("t04") || c.startsWith("t06") ||
      c.startsWith("t07") || c.startsWith("t08") || c.startsWith("t09") || c.startsWith("t10") ||
      c.startsWith("t12") || c.startsWith("t13") || c.startsWith("t14") || c.startsWith("t15") ||
      c.startsWith("t16") || (c.startsWith("t18") && !c.startsWith("t1801") && !c.startsWith("t1803") && !c.startsWith("t1805")))
      .map(new Column(_).as[Double])
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when($"tesex" === 1, "male")
      .otherwise("female").alias("sex")

    val ageProjection: Column = when($"teage" >= 15 && $"teage" <= 22, "young")
      .when($"teage" >= 23 && $"teage" <= 55, "active")
      .otherwise("elder").alias("age")

    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce(_ + _) / 60d).alias("primaryNeeds")
    val workProjection: Column = (workColumns.reduce(_ + _) / 60d).alias("work")
    val otherProjection: Column = (otherColumns.reduce(_ + _) / 60d).alias("other")
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""SELECT working, sex, age, ROUND(AVG(primaryNeeds), 1) AS primaryNeeds,
                ROUND(AVG(work), 1) AS work, ROUND(AVG(other), 1) AS work
           FROM ${viewName}
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(r =>
      new TimeUsageRow(working = r.getAs[String]("working"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val grouped = summed.groupByKey(s => (s.working, s.sex, s.age))
    ...
    .map{
      case (((workKey, sexKey, ageKey)), prim, wrk, oth) => new TimeUsageRow(working = workKey,
        sex = sexKey,
        age = ageKey,
        primaryNeeds = BigDecimal(prim).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble,
        work = BigDecimal(wrk).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble,
        other = BigDecimal(oth).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble)
    }

    grouped.withColumn("working", new Column("working"))
      .withColumn("sex", new Column("sex"))
